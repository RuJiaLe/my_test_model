...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=3, embed_dim=768):
        super().__init__()
        img_size = to_2tuple(img_size)  # 把一个数据变成二维的元组
...
class Head(nn.Module):
    def __init__(self, num):  # num = 2
        ...
        stem.append(nn.Conv2d(64, 64, kernel_size=2, stride=2))  # 大小变为一半
    ...
    def forward(self, x):  # (1, 3, 224, 224)
        x = self.conv(x)  # (1, 64, 56, 56)
        _, _, H, W = x.shape
        x = x.flatten(2).transpose(1, 2)  # flatten 参数为开始的维度
        x = self.norm(x)
        return x, H, W  # (1, 3136, 64)  56  56
...
class ShuntedTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule 在[0, drop_path_rate] 之间生成 sum(depths个数)
        cur = 0

        for i in range(num_stages):  # num_stages = 4
            if i == 0:
                patch_embed = Head(num_conv)  # (x, H, W) (1, 3136, 64), 56, 56
            else:  # 重叠Patch编码
            ...
            setattr(self, f"patch_embed{i + 1}", patch_embed)  # 对类中的属性进行操作 可以修改, 怎加类中的属性 返回值为空
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):  # 4
            ...
            for blk in block:
                x = blk(x, H, W)
            x = norm(x)
            block = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            blocks.append(block)
            if i != self.num_stages - 1:
                x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()  # torch.contiguous()方法首先拷贝了一份张量在内存中的地址，然后将地址按照形状改变后的张量的语义进行排列。
        return x.mean(dim=1), blocks
